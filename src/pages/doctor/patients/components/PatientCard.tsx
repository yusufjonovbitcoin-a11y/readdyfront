      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
            ...
            <i className={`${patient.gender === 'male' ? 'ri-user-3-line text-blue-600' : 'ri-user-3-line text-pink-600'} text-base`}></i>
          </div>
          <div>
            <h3 className={`text-sm font-semibold ${darkMode ? "text-white" : "text-gray-900"}`}>{patient.name}</h3>
            <p className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{patient.age} yosh • {patient.phone}</p>
          </div>
        </div>
