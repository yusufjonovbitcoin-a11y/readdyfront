      ...
      {showAvatar && (
        <div
          className={`w-9 h-9 rounded-xl flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-1 ${
            isMe
              ? "bg-emerald-600"
              : msg.senderId.includes("cardio")
              ? "bg-rose-600"
              : msg.senderId.includes("neuro")
              ? "bg-sky-600"
              : msg.senderId.includes("ortho")
              ? "bg-emerald-600"
              : msg.senderId.includes("ped")
              ? "bg-amber-600"
              : msg.senderId.includes("surg")
              ? "bg-violet-600"
              : "bg-pink-600"
          }`}
        >
...
export function AdminChatPageContent() {
  ...
  const [groups, setGroups] = useState<AdminChatGroup[]>([...adminChatGroups]);
  const [selectedGroupId, setSelectedGroupId] = useState<string>(adminChatGroups[0]?.id || "");
  const [messageText, setMessageText] = useState("");
  const [searchQuery, setSearchQuery] = useState("");
  const [filterHospital, setFilterHospital] = useState("Barchasi");
  ...
  const hospitals = useMemo(() => {
    const set = new Set(groups.map((g) => g.hospitalName));
    return ["Barchasi", ...Array.from(set)];
  }, [groups]);

  const filteredGroups = useMemo(() => {
    let filtered = [...groups];
    if (filterHospital !== "Barchasi") filtered = filtered.filter((g) => g.hospitalName === filterHospital);
    if (filterDept !== "Barchasi") filtered = filtered.filter((g) => g.specialty === filterDept);
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      filtered = filtered.filter((g) => g.name.toLowerCase().includes(q) || g.hospitalName.toLowerCase().includes(q) || g.specialty.toLowerCase().includes(q));
    }
    filtered.sort((a, b) => {
    ...
    return filtered;
  }, [groups, filterHospital, filterDept, searchQuery]);
  ...
  const sendMessage = () => {
    ...
    const newMsg: AdminGroupMessage = {
      id: `msg-${Date.now()}`,
      senderId: superAdminMember.id,
      senderName: superAdminMember.name,
      senderAvatar: superAdminMember.avatar,
      senderHospital: superAdminMember.hospitalName,
  ...
  const isMe = (senderId: string) => senderId === superAdminMember.id;
    ...
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
        ...
        <div className={`p-4 border-b ${darkMode ? "border-[#1E2130]" : "border-gray-100"}`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className={`text-sm font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>Barcha Guruhlar</h2>
          ...
          <div className="flex gap-2">
            <select value={filterHospital} onChange={(e) => setFilterHospital(e.target.value)} className={`flex-1 px-2.5 py-1.5 rounded-lg text-xs outline-none transition-colors cursor-pointer appearance-none ${darkMode ? "bg-[#0F1117] border border-[#1E2130] text-white" : "bg-gray-50 border border-gray-200 text-gray-700"}`}>
              {hospitals.map((h) => (<option key={h} value={h}>{h}</option>))}
            </select>
            <select value={filterDept} onChange={(e) => setFilterDept(e.target.value)} className={`flex-1 px-2.5 py-1.5 rounded-lg text-xs outline-none transition-colors cursor-pointer appearance-none ${darkMode ? "bg-[#0F1117] border border-[#1E2130] text-white" : "bg-gray-50 border border-gray-200 text-gray-700"}`}>
              {["Barchasi", ...departments].map((d) => (<option key={d} value={d}>{d}</option>))}
            </select>
          </div>
        </div>
          ...
          {filteredGroups.length > 0 ? (
            <div className="p-2 space-y-1">
              {filteredGroups.map((group) => {
                ...
                return (
                  <button key={group.id} onClick={() => { setSelectedGroupId(group.id); setShowMembers(false); setGroups((prev) => prev.map((g) => g.id === group.id ? { ...g, unreadCount: 0 } : g)); }} className={`w-full text-left rounded-xl p-3 transition-all cursor-pointer ${isActive ? (darkMode ? "bg-emerald-900/20 border border-emerald-500/30" : "bg-emerald-50 border border-emerald-200") : darkMode ? "hover:bg-[#1E2A3A]/50" : "hover:bg-gray-50"}`}>
                    <div className="flex items-start gap-3">
                      <div className="relative flex-shrink-0">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white text-xs font-bold ${specialtyColors[group.specialty] || "bg-gray-600"}`}>{group.specialty.slice(0, 2)}</div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-[11px] truncate mt-0.5 ${darkMode ? "text-gray-500" : "text-gray-400"}`}>{group.hospitalName}</p>
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${specialtyBadgeColors[group.specialty] || "bg-gray-500/10 text-gray-500"}`}>{group.specialty}</span>
                          <span className={`text-[10px] flex items-center gap-1 ${darkMode ? "text-gray-500" : "text-gray-400"}`}><span className="w-1 h-1 rounded-full bg-emerald-500" />{onlineCount} onlayn</span>
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${darkMode ? "bg-[#0F1117]" : "bg-white"}`}>
        <div className={`px-5 py-3.5 border-b flex items-center gap-3 ${darkMode ? "bg-[#141824] border-[#1E2130]" : "bg-white border-gray-100"}`}>
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${specialtyColors[selectedGroup.specialty] || "bg-gray-600"}`}>{selectedGroup.specialty.slice(0, 2)}</div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h2 className={`text-sm font-bold truncate ${darkMode ? "text-white" : "text-gray-900"}`}>{selectedGroup.name}</h2>
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0 ${specialtyBadgeColors[selectedGroup.specialty] || ""}`}>{selectedGroup.specialty}</span>
            </div>
            <p className={`text-xs truncate ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{selectedGroup.hospitalName} · {selectedGroup.members.length} a'zo · {selectedGroup.members.filter((m) => m.status === "online").length} onlayn</p>
          </div>
        ...
        <div className={`flex-1 overflow-y-auto px-5 py-5 space-y-4 ${darkMode ? "bg-[#0F1117]" : "bg-gradient-to-b from-gray-50 to-white"}`}>
          <div className="flex flex-col items-center py-4">
            <div className={`w-14 h-14 flex items-center justify-center rounded-2xl mb-3 ${darkMode ? "bg-[#1E2130]" : "bg-emerald-50"}`}><i className={`ri-chat-smile-2-line text-2xl ${darkMode ? "text-emerald-400" : "text-emerald-500"}`} /></div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-500">{selectedGroup.members.filter((m) => m.status === "online").length} onlayn</span>
            </div>
          </div>
        ...
        <div className={`px-5 py-4 border-t ${darkMode ? "bg-[#141824] border-[#1E2130]" : "bg-white border-gray-100"}`}>
          <div className="flex gap-3 items-end">
            <div className="flex-1 relative">
              <textarea value={messageText} onChange={(e) => setMessageText(e.target.value.slice(0, 500))} onKeyDown={handleKeyDown} placeholder="Xabar yozing..." rows={1} className={`w-full text-sm px-4 py-3 rounded-2xl outline-none resize-none transition-colors ${darkMode ? "bg-[#0F1117] border border-[#1E2130] text-white placeholder-gray-600 focus:border-emerald-500/50" : "bg-gray-50 border border-gray-200 text-gray-900 placeholder-gray-400 focus:border-emerald-300"}`} style={{ minHeight: "44px", maxHeight: "120px" }} />
              <p className={`absolute right-3 bottom-1 text-[10px] ${darkMode ? "text-gray-600" : "text-gray-300"}`}>{messageText.length}/500</p>
            </div>
            <button onClick={sendMessage} disabled={!messageText.trim()} className={`w-11 h-11 flex items-center justify-center rounded-xl transition-all cursor-pointer flex-shrink-0 ${messageText.trim() ? "bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm" : darkMode ? "bg-[#1E2130] text-gray-600" : "bg-gray-100 text-gray-300"}`}>
          ...
          <div className="flex-1 overflow-y-auto py-2">
            {selectedGroup.members.map((member) => (
              <div key={member.id} className={`flex items-center gap-3 px-4 py-2.5 ${darkMode ? "hover:bg-[#1E2A3A]" : "hover:bg-gray-50"}`}>
                <div className="relative flex-shrink-0">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white text-xs font-bold ${specialtyColors[member.specialty] || "bg-gray-500"}`}>{member.avatar}</div>
                  <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 ${darkMode ? "border-[#141824]" : "border-white"} ${statusColors[member.status]}`} />
                </div>
                <div className="flex-1 min-w-0">
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0 ${member.status === "online" ? "bg-emerald-500/10 text-emerald-500" : member.status === "busy" ? "bg-amber-500/10 text-amber-500" : "bg-gray-500/10 text-gray-400"}`}>{statusLabels[member.status]}</span>
              </div>
            ))}
          </div>
        </div>
